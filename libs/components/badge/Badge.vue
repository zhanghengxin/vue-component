<template>
    ...
    <span v-else-if="status" :class="[classes, `${prefixCls}-status`]" ref="badge">
    ...
    <span v-else :class="classes" ref="badge">
        <slot></slot>
        <sup v-if="hasCount" :style="styles" :class="countClasses" v-show="badge">
            {{ finalCount }}
        </sup>
    </span>
</template>
...
export default {
    ...
    props: {
        ...
        status: {
            type: String,
            validator (value) {
                return oneOf(value, ['success', 'error', 'warning', 'default', 'processing'])
            }
        },
        type: {
            type: String,
